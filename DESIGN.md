...
- Desktop dropdowns share a centered, header-relative 720px panel capped to the available width. Products uses three illustrated destination cards; Resources uses Learn (Docs, Blog, Lab) and Company (Work, About) columns with a separate All resources link. Use 16px/550 titles, 14px descriptions, soft neutral surfaces, and a subtle background on the open trigger. `navigation-panels.css` owns this layout. Mobile stacks links and hides decorative artwork. Entrance motion is a 160ms fade with 5px movement, disabled for reduced motion.

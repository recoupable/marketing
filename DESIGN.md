...
- The four top-level items are **Services, Products, Resources, Pricing**, followed by the audit action. Products contains Platform, Skills, and Developers. Resources contains Docs, Blog, Lab, About, and a featured Work card. Desktop and mobile share `headerNavigation` in `lib/copy/navigation.ts`; mobile uses nested disclosures. Only one sibling dropdown opens at a time. Links close the menus; Escape closes the innermost menu and restores focus to its visible summary.
- Desktop menus extend the full header width with no gap or separate top corners. Products uses three illustrated destination cards; Resources pairs four compact icon links with one illustrated Work card. Use 16px/550 titles, 14px descriptions, soft neutral surfaces, and a subtle background on the open trigger. `navigation-panels.css` owns this layout. Fine-pointer mouse hover opens menus, with a 180ms close delay when leaving the header; click and keyboard operation remain available. Mobile stacks links and hides decorative artwork. Entrance motion is a 160ms fade, disabled for reduced motion.
...
The homepage ownership section follows the pale numbered process with a saturated blue handoff panel (`SkyOwnership`, `sky-narrative.css`). Use a large white headline with lime emphasis on “Owned by you,” followed by three unnumbered columns for code, documentation, and team training. Fine separators and small line icons keep the deliverables readable without another set of cards. Stack them on phones, and preserve the shared ownership terms from `homeCopy`.

The `/platform` hero uses `PlatformChatPreview` in `components/platform`: a compact, upright workspace with Artists and Catalogs in the sidebar, a release header, one short exchange, a campaign-plan document, and a single-row composer. The text remains HTML; the mock controls are decorative and never focusable. Do not add a visible illustrative/fictional caption. The headline is “A ready-to-use AI workspace for music teams.” Keep the title-only narrative explicit: chat across artists/catalogs/releases, built-in music tools, hosted and open source, tools for an existing agent, and custom systems as a separate services path. Shared copy in `lib/copy/platform.ts` supplies the visible page and machine-readable summary. Below the hero, use an unboxed artist/catalog/release composition from `PlatformContextArt`, followed by a dark-green `PlatformToolsDemo`. Native pressed-state buttons switch among research, content, and recurring-work examples in a polite live region. Only the demo is a client component; it performs no network operations or real task scheduling. Hosting pairs left-aligned typography with a blue source illustration and a linked white repository card. Existing-agent and custom-service choices share one compact comparison, followed by a split FAQ. Do not restore the boxed onboarding checklist, small capability-card tray, repeated pale two-column panels, or duplicate closing banner. Stack the interactive demo and alternative paths on phones; keep decorative artwork out of the accessibility tree. Link the public app source through `siteConfig.platformSourceUrl`. Hide the sidebar on small phones so the conversation stays readable, and use the existing `AppLink` as the real action. Avoid returning to the tall workflow-selector report or tilted backplate.

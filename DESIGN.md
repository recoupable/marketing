...
The `/platform` hero uses `PlatformChatPreview` in `components/platform`: a compact, upright workspace with Artists and Catalogs in the sidebar, a release header, one short exchange, a campaign-plan document, and a single-row composer. The text remains HTML; the mock controls are decorative and never focusable. Do not add a visible illustrative/fictional caption. The headline is “AI that knows your music.” Shared copy in `lib/copy/platform.ts` covers artists, catalogs, and releases; capability cards have one heading and one description, with no repeated category labels or tags. Hide the sidebar on small phones so the conversation stays readable, and use the existing `AppLink` as the real action. Avoid returning to the tall workflow-selector report or tilted backplate.

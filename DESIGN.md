...
The `/platform` hero uses `PlatformChatPreview` in `components/platform`: a compact, upright artist chat workspace with a sidebar, artist context files, one question and answer, and a composer illustration. The text remains HTML; the mock controls are decorative and never focusable. Keep the fictional-artist caption. Hide the sidebar on small phones so the conversation stays readable, and use the existing `AppLink` as the real action. Avoid returning to the tall workflow-selector report or tilted backplate.

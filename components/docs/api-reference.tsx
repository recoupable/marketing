...
export async function ApiReference({page}:{page:DocPage}) {
 ...
 return <div className="docs-api-content">
  ...
  <section id="request-example"><h2>Request</h2><DocsCode language="bash" label="cURL">{request}</DocsCode>{parameters.some(param=>param.in==='query'||param.in==='path')&&<p className="docs-small-note">Replace the YOUR_ placeholders with your values. Required query parameters are included; optional parameters are listed below.</p>}<ApiPlayground key={`${page.api.method} ${page.api.path}`} operation={summarizeOperation({method:page.api.method,path:page.api.path,spec})} baseUrl={siteConfig.apiUrl}/></section>

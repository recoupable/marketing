...
export function PlaygroundKeyField({ auth, value, onChange }: { auth: PlaygroundAuth; value: string; onChange: (value: string) => void }) {
  const id = useId();
  if (auth.type === "none" || auth.type === "unsupported") return null;
  return (
    <div className="docs-playground-field docs-playground-key">
      ...
      <input id={id} type="password" autoComplete="new-password" spellCheck={false} data-1p-ignore value={value} onChange={(event) => onChange(event.target.value)} aria-describedby={`${id}-hint`} />

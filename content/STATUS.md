...
- **WEBSITE.md backlog FULLY IMPLEMENTED** (2026-06-13) on **`feat/research-consulting-site`** — all 37 tickets (W-01..W-37) shipped from the reference review. Conversion path moved off `mailto:` to a qualified `ContactForm` → `/api/contact` → Attio (`createAttioLead`) on consulting/partners/trust; new `/diligence` sample-report page; homepage stats strip, shared `LogoBar`, sitewide footer newsletter, manifesto interlude, objection strip, mantra closers; `/platform` receipts table + workflow chips; blog editorial beats (`lib/beats.ts`) + author bylines; verb-stamped shelf; FAQ JSON-LD on consulting/pricing/partners; branded `app/opengraph-image.tsx`; Plausible events via `lib/track.ts`. New shared components in `components/marketing/`. **Playwright e2e added** (`e2e/marketing.spec.ts`, `pnpm test:e2e`): 28 tests green on desktop + mobile. `pnpm build` green. NOTE: working tree was rolled back once mid-session (~14:29) and 10 new files had to be recreated — verify all `components/marketing/*` and `lib/copy/*` files exist after any checkout.

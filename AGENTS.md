...
The invitation pairs a short offer and **Start my audit** with `ScorecardPreview`, a clearly labeled example using the real rubric's categories and levels. Keep example ratings independent of visitor/session state and explain that questions are part of the audit. Landing styles live in `scorecard-landing.css`; conversation and report styles stay separate.

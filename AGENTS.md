...
The invitation addresses label, publishing and music-fund leaders and pairs a short offer and **Start my audit** with `ScorecardPreview`. The clearly labeled example maps concrete capabilities (catalog answers, recurring reports, team use, checked outputs and measured savings) to the real rubric's area IDs and rating levels. Keep example ratings independent of visitor/session state and explain that questions are part of the audit. Landing styles live in `scorecard-landing.css`; conversation and report styles stay separate.

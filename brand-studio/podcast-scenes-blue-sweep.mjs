import {podcastLockup as originalLockup} from './podcast-scenes-v4.mjs';
// Carry forward the approved optical lift of the icon.
const podcastLockup=ink=>originalLockup(ink).replace(' 490) scale(', ' 482) scale(');

export const FORMATS = [
  {id:'badge',label:'Podcast badge',width:1200,height:288,description:'White podcast identity on blue matched to the upper-right frame area. Use at 300 × 72 in a 1080p video.'},
  ...
  {id:'title',label:'Title card',width:1920,height:1080,description:'The Blue sweep artwork with your white, one-line podcast identity.'},
...
// Local blue near the badge position; solid fill keeps footage from competing.
export const BADGE_BLUE='#1179C9';
export const DEFAULTS={title:'AI and the\nbusiness of music',episode:'01',host:'Host name',guest:'Guest name',quote:'Good systems give people more time to think.'};
...
function paragraph(value,x,y,width,height,preferred=110){
  ...
  for(;size>=24;size-=2){
    rows=[''];
    for(const word of String(value).trim().replace(/\n/g,' __LINE_BREAK__ ').split(/\s+/)){
      if(word==='__LINE_BREAK__'){rows.push('');continue;}
...
export function renderBlueSweepSVG(options={}){
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" role="img"><title>${esc(f.label)} — Recoup Podcast</title><defs><style>${o.fontCSS||''}</style>${SWEEP_DEFS}${mask}</defs>${body}</svg>`;
}

// Geometry is recomposed for each aspect ratio; lettering is never stretched.
const SWEEP_DEFS=`<linearGradient id="azure" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#075CC5"/><stop offset="1" stop-color="#169AD7"/></linearGradient><linearGradient id="lightplane" x1="0" y1="1" x2="1" y2="0"><stop stop-color="#A9EFFF" stop-opacity="0"/><stop offset="1" stop-color="#A9EFFF" stop-opacity=".35"/></linearGradient>`;
export function blueSweepBackground(w,h){
  const d=`M${-.04*w} ${1.02*h}C${.12*w} ${.46*h} ${.44*w} ${1.38*h} ${w} ${.19*h}`;
  return `<rect width="${w}" height="${h}" fill="url(#azure)"/><path d="${d}V${h}H${-.04*w}Z" fill="url(#lightplane)"/><path d="${d}" fill="none" stroke="#C2EDFF" stroke-width="${Math.max(1.5,w/1056)}" opacity=".26"/><path d="M${.714*w} ${-.25*h}C${.606*w} ${.076*h} ${.751*w} ${.43*h} ${1.08*w} ${.164*h}" stroke="#0B4E92" stroke-width="${h*.253}" fill="none" opacity=".15"/>`;
}

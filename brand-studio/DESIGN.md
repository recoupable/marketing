...
`SkyServices` presents three buying paths: AI advisory, AI transformation, and custom builds. Its left-hand introduction states the destination and agreed success measures; three compact cards sit on the right. The transformation partnership receives lime emphasis. Small, text-free symbols support the titles; avoid large mock interfaces competing with the offer copy. A sticky desktop introduction becomes normal document flow on tablets and phones. Workflow automation, royalty reporting, catalog intelligence, and investment review appear beneath the offers as linked examples of deliverables, not separate competing engagements. The shared offer definitions in `src/lib/service-offers.ts` also drive `/services`. Training is part of the transformation engagement. Preserve the existing services-page strategy, build, and enable anchors. Reuse `sky-services.css`; cards use the shared scroll entrance controller.

After the case studies, `SkyPartnership` presents four equal numbered cards below a full-width start-to-finish headline and audit CTA. Cards show discovery, roadmap, build, and launch/adoption; four columns become two on tablets and one on phones. Services explain what clients buy; the process explains how work unfolds.

`HomeCaseStudies` is a dark-green portfolio section with a featured royalty-reporting project above two supporting investment and catalog stories. Each pairs the reusable `CaseArt` output illustration with its audience, literal project title, concise documented outcome, and capability badges. Outcome badges describe supported workflow changes; they must not invent performance metrics. The featured project pairs art and copy horizontally on desktop; all projects stack on phones. Use `home-case-studies.css` for this section. Do not use an individual report to explain the entire transformation offer. The full interactive royalty and investment walkthroughs remain on `/operations` and `/acquisitions`. Preserve homepage anchors `#case-studies`, `#work`, `#royalty-example`, `#acquisition-example`, `#services`, and `#tools`; example anchors lead to their case-study previews.
...
`SkyEngagements` uses two complete comparison cards: pale Advisory for teams implementing themselves and dark-green Build + Partner for hands-on delivery. Each card shows the audience, prominent price and billing terms, direct plan inquiry, shared inclusions, and scope. Monthly is the default; accessible native radios switch to the shared annual pricing calculation and preserve the selection in the project form. Keep the free audit and full comparison below, followed by a distinct self-serve tools section. Stack complete cards at mobile widths.

Engagement prices use `planPrice` from `src/lib/pricing.ts`, never duplicated price constants. Software is explicitly a separate self-serve option. `SkySoftwareStrip` in `sky-tools.tsx` presents a featured Platform card with an illustrative artist workspace, followed by open-source Skills and API/MCP/CLI cards with playbook and connection artwork. Use the shared platform price, explain that consulting is not required, and keep service-usage notes visible. Decorative illustrations are hidden from assistive technology; actual actions sit in the card copy. Reuse `sky-tools.css` and stack all three paths on mobile. The primary CTA is **Get a Free Audit**, including the closing banner. Ownership language applies to custom systems; explain repository control, documentation, training, and agreed licenses/dependencies rather than implying ownership of third-party platforms.
...
Start with [the shared Sky components](src/components/sky/page-ui.tsx) and [their stylesheet](src/components/sky/site.css). For homepage work, use [the route and metadata](src/app/page.tsx), [the homepage](src/components/home/home-page.tsx), [its composition](src/components/home/sky-content.tsx), and their imported modules. The [research notes](docs/design/2026-09-10-design-md-research.md) record the original review; their earlier source paths predate the cleanup.
...
Use **DM Sans Variable** for headings, body, and the wordmark, and **IBM Plex Mono 400** for compact labels, code, and technical metadata. Both are locally loaded in [the root layout](src/app/layout.tsx). The logo reads **Recoup**, capital R, weight **600**. Use the existing symbol and wordmark components. Both footer variants use `FooterBrand` from `src/components/sky/brand.tsx`, with a 23×28px symbol, 11px gap, and a 28px/600 wordmark (26px on phones). Header-only logo rules must be scoped to `.ss-header` or `.sky-nav`; their sizing variables and mobile icon-only layout must never affect a footer.
...
| `src/app/blog/blog-art.tsx` | Existing illustrated article-cover vocabulary; inspect it before commissioning more imagery. |
...
The homepage statement has its own reading interval between the customer logos and services: at least 90svh on desktop, with generous space above and below the paragraph. On phones, use 112px above and 136px below without forcing a viewport height. Keep normal scrolling and compact print spacing.
...
| --- | --- |
| Public-page shell | [SiteFrame](src/components/site-frame.tsx), [SkySiteHeader](src/components/sky/site-header.tsx), [SkySiteFooter](src/components/sky/site-footer.tsx) |
| Hero / section / action / closing CTA | `PageHero`, `PageSection`, `PageButton`, `PageCTA` in [page-ui.tsx](src/components/sky/page-ui.tsx) |
| Logo and arrows | [PageMark](src/components/sky/brand.tsx), [SkyArrow](src/components/sky/arrow.tsx) |
| Hero scene | [SkyHeroCards](src/components/home/sky-hero-cards.tsx) and [its content styling](src/components/home/sky-hero-cards.css) |
| Problem-to-outcome comparison | [SkyTransformation](src/components/home/sky-transformation.tsx) |
| Source-linked report | [SkyRoyaltyExample](src/components/home/sky-royalty-example.tsx) and [shared data logic](src/lib/sky-royalty-example.ts) |
| Diligence composition | [SkyInvestmentExample](src/components/home/sky-investment-example.tsx) |
| Engagements / tools | [SkyPartnership and SkyTools](src/components/home/sky-partnership.tsx) |
| Articles / reading | [blog-art.tsx](src/app/blog/blog-art.tsx), [blog.css](src/app/blog/blog.css), [docs.css](src/app/docs/docs.css) |
| Inquiry and agent handoff | [InquiryForm](src/components/inquiry-form.tsx), [browser-agent-tools.tsx](src/components/agents/browser-agent-tools.tsx) |
...
The [full-site visual review](docs/design/full-site-visual-review.md) and [page-by-page section inventory](docs/design/full-site-section-inventory.md) record the subsequent desktop, mobile, reading, and interaction pass. These dated records predate removal of the historical design routes and the move of homepage components into `src/components/home`; their archived route lists are not current site inventory.

...
def vinyl(x,y,r=180,stroke=INK,fill=PAPER):
 ...
 return s
CONTENT=[
 ['Start with','the work','on repeat.'],['The report','isn’t the','whole job.'],['Give it','one place','to start.'],['Let AI','prepare.','You decide.'],['Try one','real cycle.'],['What does','your team','do on repeat?']]
SUB=[['A useful first step with AI.'],['First comes finding','everything that goes into it.'],['Bring the inputs together.','Then ask AI to help.'],['A draft is a starting point.','Your team makes the call.'],['Check the result.','Fix the gaps. Repeat.'],['One task. One owner.','One useful first step.']]
STEPS=['THE START','THE FRICTION','THE INPUTS','THE REVIEW','THE TEST','YOUR TURN']
...
def blue(n):
 ...
 s+=lines(CONTENT[n],72,330,94,PAPER)+lines(SUB[n],74,710 if n not in [4] else 615,34,PALE,1.22)
 if n==0:
  ...
  s+=text('Again. And again.',84,1138,30,PAPER)
 elif n==1:
  ...
  s+=text('Start there.',260,1100,87,PAPER)+path('M888 1090l30-66 22 67',LIME,5)
 return s+footer(n,PAPER)
def paper(n):
 s=rect(0,0,1080,1350,PAPER)+wordmark(INK)+text(STEPS[n],770,94,22,INK)
 s+=path('M72 145H1008',INK,1)
 if n==0:
  s+=lines(CONTENT[n],72,295,100)+text('A useful first step with AI.',76,655,34)
  ...
  s+=lines(['The report isn’t','the whole job.'],72,284,78)
  ...
  s+=lines(CONTENT[n],72,282,90)
  s+=circle(838,668,239,PALE)
  ...
  s+=lines(['Let AI prepare.'],72,302,88)+lines(['You decide.'],72,405,88,BLUE)
  ...
  s+=lines(CONTENT[n],72,430,96)+lines(SUB[n],75,885,39)
  s+=text('Start there.',74,1155,72,BLUE)+arrow(800,1125,175,BLUE)
...
def signal(n):
 ...
 if n==0:
  for r in range(590,79,-65):s+=circle(1095,1030,r,BLUE if r%130==70 else 'none',INK,2)
  s+=lines(CONTENT[n],72,340,106)+lines(SUB[n],75,748,36)
 elif n==1:
  for i in range(7):
   x=80+i*145;y=900+(i%2)*70
   s+=rect(x,y,95,195,BLUE if i%2 else '#2B594D',0)
   s+=path(f'M{x+22} {y+44}h48m-48 27h36',LIME,2)
  s+=lines(['The report isn’t','the whole job.'],72,335,81,fg)+lines(SUB[n],74,625,38,PALE)
 elif n==2:
  s+=lines(CONTENT[n],72,335,99)
  ...
  s+=lines(['Let AI','prepare.'],72,326,111,PAPER)
  s+=lines(['You decide.'],72,927,104,LIME)
  s+=circle(903,647,106,LIME)+path('M854 646l34 33 65-74',INK,7)
  s+=lines(SUB[n],75,1084,35,PAPER)
 elif n==4:
  s+=lines(CONTENT[n],72,340,108)
  ...
  s+=text('Learn. Then repeat.',72,663,37)
 else:
  for i in range(12):s+=path(f'M{790+i*30} -20L{590+i*30} 350',BLUE,12)
  s+=lines(CONTENT[n],72,506,101,PAPER)+lines(SUB[n],75,940,39,PALE)
  s+=text('Start there.',75,1164,73,LIME)+arrow(820,1134,150,LIME)
...
(ROOT/'carousel-content.json').write_text(json.dumps([dict(title=' '.join(a),body=' '.join(b),step=c) for a,b,c in zip(CONTENT,SUB,STEPS)],indent=2)+'\n')

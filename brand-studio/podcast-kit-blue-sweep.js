import {FORMATS,DEFAULTS,CAMERA_WINDOWS,renderBlueSweepSVG} from './podcast-scenes-blue-sweep.mjs';
...
const asset='assets/podcast-kit-blue-sweep/';
...
function source(extra={}){return renderBlueSweepSVG({...state,backgroundHref,fontCSS,...extra});}
function render(){
...
async function download(kind){
  ...
  try{
    ...
    const blob=new Blob([source({preview:false,labelsOnly})],{type:'image/svg+xml'}),name=`recoup-podcast-blue-sweep-${f.id}${labelsOnly?'-labels':''}`;
...
async function start(){
  ...
  fontCSS=await Promise.all([['DM Sans','dm-sans.woff2'],['IBM Plex Mono','ibm-plex-mono.woff2']].map(async([name,file])=>`@font-face{font-family:'${name}';src:url('${await dataURI('assets/fonts/'+file)}') format('woff2');font-weight:100 1000;}`)).then(a=>a.join(''));
  for(const [key,value] of Object.entries(DEFAULTS)){const field=document.querySelector(`[name="${key}"]`);if(field)field.value=value;}

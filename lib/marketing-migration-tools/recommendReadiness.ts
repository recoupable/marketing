...
/** The first failed gate picks the next step. */
export function recommendReadiness(answers: Record<string,string>) {
  const { dataNeedsWork, unclearWork, ownerNeedsWork } = readinessGates(answers);
  return dataNeedsWork ? recommendations.information : unclearWork ? recommendations.work : ownerNeedsWork ? recommendations.owner : recommendations.build;
}

...
- The background stays pinned while the cloud image grows. Its position and scale settle before the first mission line enters the viewport, then stay fixed throughout the paragraph reveal and fade to white. The foreground fades independently.
- The larger white mission arrives 16svh earlier on desktop (20svh on phones). Each word sharpens from a 7px blur and 35% opacity to solid white in reading order. Overlapping word reveals move the focus smoothly across each line while upcoming words remain softly visible. The reveal spans roughly one viewport unit, then holds for only 0.12 phase units (12svh on desktop, about 9svh on phones) before the clouds fade to white. The opening cloud move uses the measured paragraph height to finish before the text appears, keeping a stable cloud composition around the text.
...
`SkyScrollPreview` uses Motion scroll values for transforms and opacity. `useSkyWordReveal` staggers the existing statement word spans in reading order and updates their blur and opacity from the same scroll phase, without measuring line breaks or reading layout on scroll. A ResizeObserver measures the responsive hero. The scene travel includes the earlier mission entrance and brief reading hold. The services handoff advances together with the sky fade, without changing the word reveal speed. The scene clips its overlapping sticky backdrop at its own boundary so the white fade layer cannot cover the following service cards. Native wheel, touch, keyboard, fragment navigation, and reverse scrolling retain their behavior; no scroll interception or animation timers are used.

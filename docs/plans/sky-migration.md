...
| App destinations | Config currently uses `teams.recoupable.dev` | Shared config uses `chat.recoupable.dev`; valuation links to `teams` | Verify the intended destination per flow and preserve catalog/checkout deep-link parameters. |
...
6. Reconcile active marketing PRs before merging. Particularly: #89 music-video offer and quote form, #90 analysis caps, #88 pricing/signed-in state, #78 attribution, #66 customer-claim corrections, and valuation/share work. Do not blindly merge their old layouts or discard their business logic.

...
export async function generateMetadata({params}:Props):Promise<Metadata>{const {slug=[]}=await params;const key=slug.join('/');const page=docs.find(item=>item.slug===key);if(!page&&key!=='api-reference')notFound();return documentationMetadata(page,key,site.url);}

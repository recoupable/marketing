...
function useReveal() {
  ...
  return {
    ref,
    cls: `transition-all duration-[900ms] ease-[cubic-bezier(.16,1,.3,1)] ${v ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`,
  };
}
...
function useStagger(n: number, ms = 130) {
  ...
  return {
    ...
    item: (i: number) => ({
      ...
      style: {
        transitionDelay: go ? `${i * ms}ms` : "0ms",
...
function FloatingPills({ show }: { show: boolean }) {
  return (
    <div
      className="absolute inset-x-0 top-0 h-screen z-[1] pointer-events-none hidden lg:block"
      aria-hidden="true"
    >
      {FLOATING_AGENTS.map((agent, i) => (
        <span
          ...
          style={{
            ...
            animation: show
              ? `float-pill ${4 + (i % 4)}s ease-in-out ${agent.delay}ms infinite alternate`
              : "none",
          }}
        >
          {agent.label}
    ...
    <div className="bg-(--background) text-(--foreground) overflow-x-hidden">
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-6 sm:px-10">
          <div className="pb-8 flex flex-col items-center text-center">
            <div className="hero-text max-w-[800px] mx-auto flex flex-col items-center transition-all duration-500 ease-[cubic-bezier(.25,1,.5,1)] origin-top">
              <span
                className={`inline-flex items-center gap-2.5 mb-5 px-4 py-2 rounded-full text-[12px] uppercase tracking-[0.16em] font-pixel text-(--foreground)/50 transition-all duration-700 ease-[cubic-bezier(.16,1,.3,1)] ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{
              ...
              <h1
                className={`font-pixel text-[clamp(3rem,9vw,6rem)] text-(--foreground) leading-[0.95] tracking-[-0.01em] mb-4 transition-all duration-1000 ease-[cubic-bezier(.16,1,.3,1)] ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
              ...
              <p
                className={`text-(--foreground)/50 text-[clamp(1.0625rem,1.6vw,1.25rem)] mb-9 leading-[1.6] font-display italic transition-all duration-900 ease-[cubic-bezier(.16,1,.3,1)] ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
            ...
            <div
              className={`w-full transition-all duration-500 ease-[cubic-bezier(.25,1,.5,1)] ${show ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{ transitionDelay: show ? "750ms" : "0ms" }}
              id="hero-valuation-wrapper"
            >
              <CatalogValuation />
            </div>
          </div>
      ...
      <section className="py-8 border-y border-(--border)">
        <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-center flex-wrap gap-x-8 gap-y-3">
          <span className="font-ui text-[10px] text-(--foreground)/20 uppercase tracking-[0.15em]">
            Used by
          </span>
          {LOGOS.map((n) => (
            <span
              key={n}
              className="font-ui font-bold text-[12px] text-(--foreground)/20 uppercase tracking-[0.06em] select-none"
            >
              {n}
            </span>
          ))}
      ...
      <section className="py-24 sm:py-32 bg-(--muted)/40">
        <div
          ref={research.ref}
          className={`max-w-[1100px] mx-auto px-6 sm:px-10 ${research.cls}`}
        >
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div>
              ...
              <p className="text-[15px] text-(--foreground)/45 leading-relaxed mb-6 max-w-md">
                Streaming metrics, audience demographics, playlist placements,
                social traction, career history — pulled from 50+ sources and
      ...
      <section className="py-24 sm:py-32">
        <div
          ref={content.ref}
          className={`max-w-[1100px] mx-auto px-6 sm:px-10 ${content.cls}`}
        >
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div className="order-1 lg:order-2">
              ...
              <p className="text-[15px] text-(--foreground)/45 leading-relaxed mb-6 max-w-md">
                Videos, images, captions, press copy — generated in batches from
                your catalog. Fat Beats created 22 finished videos in 2 hours
                with zero editing.
              </p>
              <p className="text-[13px] text-(--foreground)/30 italic">
      ...
      <section className="py-24 sm:py-32 dark-section text-white relative overflow-hidden">
        <div
          ref={arch.ref}
          className={`max-w-[1100px] mx-auto px-6 sm:px-10 relative z-10 ${arch.cls}`}
        >
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-3 mb-5">
              {["API", "MCP", "CLI"].map((m) => (
                <span
                  key={m}
                  className="text-[10px] font-pixel text-white/30 uppercase tracking-[0.15em] px-3 py-1.5 rounded-full border border-white/10"
                >
                  {m}
                </span>
              ))}
            ...
            <p className="text-[15px] text-white/35 max-w-lg mx-auto leading-relaxed">
              Works with Claude, ChatGPT, Cursor, Windsurf, and any
              MCP-compatible agent. Or just use our chat.
            </p>
          </div>
      ...
      <section className="relative py-32 sm:py-44 overflow-hidden dark-section-cta">
        ...
        <div
          ref={cta.ref}
          className={`max-w-[700px] mx-auto px-6 text-center relative z-10 ${cta.cls}`}
        >

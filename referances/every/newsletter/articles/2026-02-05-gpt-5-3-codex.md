...
subtitle: "The autonomy we wanted is here—but the model still does what you say, not what you mean"
date: 2026-02-05
column: vibe-check
url: https://every.to/vibe-check/gpt-5-3-codex
paywalled: false
layout: special
scraped_at: 2026-06-11T16:07:23.794Z
---
...
It has the frontier coding chops of the company's latest research combined with GPT-5.2's reliability for agentic work. It's built for long-horizon tasks&mdash;the kind of sustained, multi-step work that unfolds over minutes or hours rather than a single prompt-and-response.
...
[Subscribe to unlock &rarr;](https://every.to/subscribe?utm_source=ash_vibecheck)

Finding 01

### Codex finally stops asking for permission

Dan tested Codex 5.3 on Proof, a macOS markdown editor that he's been vibe coding that tracks the origin of every piece of text&mdash;whether it was written by a human or generated by AI&mdash;and lets users attest to how thoroughly they've reviewed AI-generated content.
...
### It still does exactly what you say&mdash;for better or worse
...
The benchmark is named after the `/lfg` command in Every's [compound engineering plugin](https://github.com/EveryInc/compound-engineering-plugin)&mdash;a single command that kicks off an entire development workflow. You give it one reasonably detailed but high-level prompt, and it handles the rest: planning what to build, writing the code, and reviewing its own work. There's no hand-holding or step-by-step guidance. The model either figures it out and delivers working code, or it doesn't.
...
**On consistency, Codex was shakier.** One earnings dashboard run produced zero output files despite reporting success&mdash;a critical reliability issue. The e-commerce task sometimes generated completely different project structures from the same prompt.

**On code quality, Codex was solid.** The code it produced was clean and well-organized&mdash;among the best in our tests.

### A note on our benchmarks

The benchmarks may tell us as much about task design as model capability. The instructions in our benchmarks are thorough but not full-specification&ndash;detailed (hence the "LFG" moniker). We want to know which models can figure it out on their own.
...
This matches what we've observed in daily use. Codex tackled Naveen's redesign task&mdash;which came with clear requirements&mdash;without build errors. Claude Opus 4.6 loves autonomous planning tasks like Kieran's. The benchmarks don't tell the whole story; they tell a story, one where exploratory problem-solving matters more than precise execution.
...
Naveen's experience bears this out. He gave both Opus 4.6 and GPT-5.3 Codex a big redesign task&mdash;one with clear requirements. Codex handled it with no build errors. Opus couldn't finish. The difference was that Naveen knew exactly what he wanted and wrote it down. That's where Codex excels.

### Great at following docs

Andrey asked GPT-5.3 Codex to build an MCP server&mdash;MCP (Model Context Protocol) being the standard that lets AI models connect to external tools and data sources. Codex read the documentation on its own and nailed the implementation, which speaks to one of the model's genuine strengths: hand it a spec or a set of docs and it'll build exactly what they describe.

### Noticeably faster&mdash;enough to change how you use it

GPT-5.3 Codex is noticeably faster than 5.2, though still not as fast as Cursor's Composer 1 Alpha. For Kieran, whose compounding engineering workflow depends on fast iteration loops, the speed gain was the thing that surprised him most about the release&mdash;enough to shift Codex from a model he'd tolerate to one he'd reach for.
...
The flipside of the caution fix. Earlier Codex versions would have stopped and asked when they started drifting from your intent. GPT-5.3 Codex keeps going&mdash;great when it's heading in the right direction, frustrating when it's not.

On longer tasks, the model sometimes gets lost down rabbit holes. It's technically executing on each step, but gradually moving away from the original intent&mdash;solving the problem it thinks you have, which may not be the problem you actually have.
...
[Get your first 15 days free &rarr;](https://every.to/subscribe?utm_source=ash_vibecheck)
